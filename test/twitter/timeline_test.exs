defmodule Twitter.TimelineTest do
  ...
  describe "posts" do
    ...
    import Twitter.TimelineFixtures

    @invalid_attrs %{body: nil, likes_cout: nil, reposts_count: nil, username: nil}
    ...
    test "create_post/1 with valid data creates a post" do
      valid_attrs = %{body: "some body", likes_cout: 42, reposts_count: 42, username: "some username"}

      assert {:ok, %Post{} = post} = Timeline.create_post(valid_attrs)
      assert post.body == "some body"
      assert post.likes_cout == 42
    ...
    test "update_post/2 with valid data updates the post" do
      post = post_fixture()
      update_attrs = %{body: "some updated body", likes_cout: 43, reposts_count: 43, username: "some updated username"}

      assert {:ok, %Post{} = post} = Timeline.update_post(post, update_attrs)
      assert post.body == "some updated body"
      assert post.likes_cout == 43
